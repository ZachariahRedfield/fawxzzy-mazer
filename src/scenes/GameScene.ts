...
export class GameScene extends Phaser.Scene {
  ...
  private bootstrapRun(seed: number): void {
    ...
    this.maze = generateMaze({
      ...
      shortcutCountModifier: legacyTuning.board.shortcutCountModifier.inRun
    });

    const camScaleForBoard = Phaser.Math.Clamp(
      legacyTuning.camera.scale.default,
      legacyTuning.camera.scale.min,
      legacyTuning.camera.scale.max
    );
    const layout = createBoardLayout(this, this.maze, {
      boardScale: legacyTuning.game.boardScale + (camScaleForBoard * legacyTuning.camera.boardScalePerStep),
      topReserve: legacyTuning.game.topReserve,
      bottomPadding: legacyTuning.game.bottomPadding
    });
